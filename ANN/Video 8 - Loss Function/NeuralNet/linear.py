...
class Linear:
    """
    Kelas untuk merepresentasikan layer linear (fully connected layer) dalam neural network.

    Layer linear melakukan transformasi linear pada input menggunakan weight dan bias.
    Transformasi ini adalah dasar dari banyak model machine learning, termasuk neural network.

    Rumus transformasi linear:
        Z = X * W + B
    di mana:
        - Z: Output dari layer linear
        - X: Input matrix (m x ℓ)
        - W: Weight matrix (ℓ x n)
        - B: Bias vector (1 x n)

    Atribut:
        in_features (int): Jumlah fitur input (ℓ).
        out_features (int): Jumlah neuron dalam layer (n).
        weight (np.ndarray): Matriks weight dengan bentuk (ℓ x n).
        bias (np.ndarray): Vektor bias dengan bentuk (1 x n).
    """
    ...
    def __init__(self, in_features: int, out_features: int) -> None:
        ...
        self.weight = np.random.randn(in_features, out_features) * 0.01  # Matriks weight (ℓ x n)
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        return np.dot(x, self.weight) + self.bias  # Z(m, n) = X(m, ℓ) * W(ℓ, n) + B(1, n)